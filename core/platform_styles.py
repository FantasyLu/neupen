"""
平台风格配置模块
存储各平台/标签的量化风格档案（与 Novel.style_profile 结构完全一致）。
用户在风格设置页以此为初始值，修改后保存为项目自己的风格档案。
LLM 注入只使用 Novel.style_profile，本模块不再向 Agent 提供注入内容。
"""
...
# ======================================
# 内置默认风格档案（首次使用时写入 JSON）
# 结构：{平台名: {标签名: style_profile_dict}}
# style_profile_dict 字段与 Novel.style_profile 完全一致
# ======================================
...
def load_platform_styles() -> dict[str, dict[str, dict]]:
    ...
    if PLATFORM_STYLES_FILE.exists():
        try:
            data = json.loads(PLATFORM_STYLES_FILE.read_text(encoding="utf-8"))
            # 检测是否为旧格式（标签值为字符串）
            for platform, tags in data.items():
                for tag, val in tags.items():
                    if isinstance(val, str):
                        # 旧格式：重置为新格式默认值
                        save_platform_styles(DEFAULT_PLATFORM_STYLES)
                        return dict(DEFAULT_PLATFORM_STYLES)
                    break
                break
            return data
        except Exception:
...
def save_platform_styles(styles: dict[str, dict[str, dict]]):
    """将平台风格配置保存到 JSON 文件"""
...
def get_platform_slider_defaults(platform: str, tags: list[str]) -> dict | None:
    """
    返回指定平台+标签组合的风格档案（多标签时量化维度取均值，文本维度取第一个匹配）。
    找不到匹配时返回 None。
    """
    if not platform or not tags:
        return None
    styles = load_platform_styles()
    platform_map = styles.get(platform, {})
    ...
    for f, _ in SLIDER_FIELDS:
        vals = [m[f] for m in matched if isinstance(m.get(f), int)]
        if vals:
            result[f] = max(1, min(5, round(sum(vals) / len(vals))))
    return result


def get_style_description(platform: str, tags: list[str]) -> str:
    """已废弃：平台风格不再作为独立文本注入 LLM，保留此函数避免旧调用报错。"""
    return ""
