...
每章写作或手动润色完成后，自动执行以下三步后处理：

**Step 1 — 破折号滥用程序化修复**

无 LLM 成本，正则直接替换。保护合法用法（话语打断、音效延长），其余场景（解释说明、递进强调、镜头切换、心理转折）一律改为逗号或句号。

**Step 2 — 禁止句式 LLM 修正**

检测到"不是……而是……""与其说……不如说……"等句式时，发起一次 LLM 修正，将对比转折结构拆分为独立事实陈述，并进行二次验证。
...
| 执行顺序 | 比喻精简在润色 LLM 调用**之前**执行，避免润色时重新引入无效比喻 |
